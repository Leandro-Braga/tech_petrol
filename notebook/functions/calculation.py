...
def data_inicio_fim_dataframe(df, start_date, end_date) -> pd.DataFrame:
    
    # ler arquivo excel que esta na pasta
    df_dados_ipea = df

    # converter o campo date para Data
    # df_dados_ipea['Data'] = pd.to_datetime(df_dados_ipea['DATA'])
    
    start_date = pd.to_datetime(start_date)
    end_date = pd.to_datetime(end_date)
    
    # selecionar o range de datas do dataframe
    df_dados_ipea = df_dados_ipea[(df_dados_ipea['DATA'] >= start_date) & (df_dados_ipea['DATA'] <= end_date)]
    
    df_dados_ipea.set_index('DATA', inplace=True)
    
    return df_dados_ipea
...
def renomeia_colunas(df):
    ...
    df_fechamento['unique_id'] = 'PETROL'
...
def calcular_e_exibir_diferenca(forecast_df):
    ...
    try:
        ...
        dif = forecast_df[['ds', 'trend', 'yhat']].rename(columns={'ds': 'Data', 'trend': 'Tendência', 'yhat': 'Valor Previsto'})

        # Calcula a diferença percentual absoluta entre os valores reais e previstos
        dif['Diferença Percentual (%)'] = np.abs((1 - (dif['Tendência'] / dif['Valor Previsto']))) * 100
    
    return dif


def calcular_e_exibir_diferenca_pkl(forecast_df):
    # Seleciona as colunas relevantes e renomeia para nomes mais descritivos
    dif = forecast_df[['ds', 'yhat']].rename(columns={'ds': 'Data', 'yhat': 'Valor Previsto'})
